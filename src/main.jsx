...
const projects = [
  { title: 'Limpieza y análisis de datos', description: 'Proyecto de análisis de datos con limpieza y transformación de información mediante Python, seguido de la creación de un dashboard interactivo en Power BI.', tags: ['Python', 'Power BI', 'Análisis de datos'], link: 'https://github.com/fernandezmaximiliano255-spec/portfolio-data' },
...
function App() {
  return <main>
    ...
    <section className="projects shell" id="proyectos"><div className="section-heading"><div><p className="eyebrow">Trabajos realizados</p><h2>Proyectos y análisis</h2></div><a className="text-link" href="/cv.pdf" target="_blank" rel="noreferrer">Ver CV ↗</a></div><div className="project-list">{projects.map((project, index) => { const Card = project.link ? 'a' : 'article'; return <Card className="project-card" key={project.title} href={project.link} target={project.link ? '_blank' : undefined} rel={project.link ? 'noreferrer' : undefined}><div className="project-number">01 / 0{index + 1}</div><h3>{project.title}</h3><p className="muted">{project.description}</p><div className="tags">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>{project.link && <span className="project-link">Ver proyecto en GitHub ↗</span>}</Card>; })}</div></section>
